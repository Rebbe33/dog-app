...
export default function EducationBaseDetail() {
  ...
  if (!trick) return <p className="text-sm text-ink/50">Compétence introuvable.</p>

  const validated = steps.filter((s) => s.completed).length

  return (
    <div className="space-y-6">
      <Link to="/education" className="inline-flex items-center gap-1 text-sm text-ink/50">
        <ArrowLeft size={14} /> Éducation de base
      </Link>
      ...
      <div>
        ...
        {steps.length === 0 && (
          <p className="text-sm text-ink/50 mb-2">
            Pas encore d'étapes — ajoute une progression personnalisée pour cette compétence.
          </p>
        )}
