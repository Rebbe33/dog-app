  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-xl font-semibold text-ink">Tours</h2>
        <span className="text-xs text-ink/40 font-mono">{tricks.length} tours</span>
      </div>
      ...
      <ul className="space-y-2">
        {filtered.map((t) => {
          ...
          return (
            <li key={t.id}>
              <Link to={`/tours/${t.id}`} className="card !py-3 flex items-center justify-between">
                <div>
      ...
      {filtered.length === 0 && (
        <p className="text-sm text-ink/50">Aucun tour ne correspond à ta recherche.</p>
      )}
    </div>
  )
}
